...
const Media = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:3001/api/img/get-all');
  ...
  const handleSave = async () => {
    ...
    try {
      if (activeTab === 'url') {
        ...
        await fetch(`http://localhost:3001/api/img/update/${editingImage._id}`, {
          method: 'PUT',
          headers: {
  ...
  const handleConfirmDelete = async () => {
    try {
      await fetch(`http://localhost:3001/api/img/delete/${deletingImageId}`, {
        method: 'DELETE',
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'url',
      width: 200,
      render: (text) => <img src={text} alt="image" style={{ width: '60px', height: '60px' }} />,
    },
    {
      title: 'Action',
      key: 'action',
